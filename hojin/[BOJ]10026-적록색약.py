...
n=int(input())
drawing=[input().rstrip() for _ in range(n)]
visited=[[False]*len(drawing[0]) for _ in range(n)]
...
def bfs(x,y):
    ...
    while q:
        ...
        for i in range(4):
            nx=dx[i]+x
            ny=dy[i]+y
            if 0<=nx<len(drawing[0]) and 0<=ny<n and (not visited[ny][nx]):
                if drawing[ny][nx]==drawing[y][x]:
                    q.append((nx,ny))
                    visited[ny][nx]=True
...
for i in range(n):
    drawing[i]=drawing[i].replace("G","R")
